...
dict_report = {
    f"Пользователей системы: {unique_use}",
    f"Процессов запущено: {result_count}",
    f"Пользовательских процессов: {dictionary}",
    f"Всего MEM используется: {sum_m_format}",
    f"Всего CPU используется: {sum_c_format}",
    f"Больше всего MEM использует: {max_mem} {Path(max_mem_proc).name[:20]}",
    f"Больше всего CPU использует: {max_cpu} {Path(max_cpu_proc).name[:20]}"
}
result = '\n'.join(dict_report)
